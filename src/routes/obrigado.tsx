...
import { Copy, Check, ArrowRight, Brain, Calendar as CalendarIcon, Compass as CompassIcon, ShieldCheck, Clock } from "lucide-react";
...
function TypewriterText({ text, className }: { text: string; className?: string }) {
  ...
  return (
    <span className={className}>
      {displayed}
      <motion.span animate={{ opacity: [1, 0] }} transition={{ repeat: Infinity, duration: 0.8 }} className="ml-0.5">_</motion.span>
    </span>
  );
}
...
function ThankYouPage() {
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-black p-4 text-white">
        <div className="rounded-2xl border border-[#CC0000]/40 bg-[#CC0000]/10 p-8 text-center max-w-md backdrop-blur-xl">
          ...
          <Link to="/" className="mt-6 inline-block rounded-full bg-[#CC0000] px-6 py-3 font-bold text-white transition hover:scale-105">
            Voltar ao início
          </Link>
        </div>
      </div>
  ...
  const steps = [
    { icon: Brain, title: "Acesse o Dashboard", desc: "Veja seu painel personalizado com tudo organizado", href: "/dashboard", color: "from-blue-500/20 to-blue-600/10" },
    { icon: CalendarIcon, title: "Complete o Onboarding", desc: "7 perguntas rápidas para calibrar seu protocolo", href: "/onboarding", color: "from-amber-500/20 to-amber-600/10" },
    { icon: CompassIcon, title: "Comece seu Diagnóstico", desc: "IA gera sua análise comportamental completa", href: "/dashboard/diagnosis", color: "from-emerald-500/20 to-emerald-600/10" },
  ];
  ...
  return (
    <div className="min-h-screen bg-black text-white relative overflow-hidden">
      ...
      <div className="relative z-10 mx-auto max-w-3xl px-4 py-12 md:py-20">
        ...
        >
          <Link to="/" className="inline-flex items-center gap-2 font-display text-2xl font-black tracking-tighter mb-8">
          ...
          <p className="mt-4 text-lg text-gray-400 leading-relaxed max-w-xl mx-auto">
            Sua jornada de transformação comportamental começa agora. Estamos felizes em ter você connosco.
          </p>
        </motion.div>
        ...
        >
          ...
          <div className="space-y-4">
            <div className="rounded-xl border border-white/5 bg-white/5 p-4">
              <label className="text-[10px] font-black uppercase tracking-[0.3em] text-gray-500 block mb-1">Email</label>
              <p className="text-lg font-bold text-white break-all">{email || "Email não disponível"}</p>
            </div>

            <div className="rounded-xl border border-[#CC0000]/20 bg-[#CC0000]/5 p-4">
              <label className="text-[10px] font-black uppercase tracking-[0.3em] text-gray-500 block mb-1">Senha padrão</label>
              <div className="flex items-center justify-between gap-3">
                <p className="text-xl font-mono font-bold text-[#CC0000] tracking-wider">{defaultPassword}</p>
                <button
              ...
              <p className="mt-2 text-xs text-gray-500">
                Você pode alterar sua senha a qualquer momento em <strong className="text-gray-400">Configurações → Segurança</strong> dentro do painel.
              </p>
            </div>
          </div>
        </motion.div>
        ...
        <motion.div
          ...
          className="mb-12"
        >
          <h2 className="font-display text-2xl font-black text-center mb-8 tracking-tight">O que fazer agora?</h2>
          <div className="space-y-4">
            {steps.map((step, i) => (
              <motion.div
                key={i}
              ...
              >
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <h3 className="font-bold text-lg text-white group-hover:text-[#CC0000] transition-colors">{step.title}</h3>
        ...
        >
          ...
          <div className="space-y-3">
            {[
              { q: "Como faço login?", a: "Use o email da compra com a senha MindReset2026! (que você pode copiar acima). Se esquecer a senha, use 'Esqueci a senha' na tela de login." },
              { q: "O que fazer primeiro?", a: "Complete o Onboarding (7 perguntas rápidas). Depois, acesse o Diagnóstico para receber sua análise comportamental personalizada por IA." },
              { q: "Preciso de ajuda?", a: "Acesse nosso suporte por email ou chat. Estamos aqui para ajudar você a ter a melhor experiência possível." },
            ].map((item, i) => (
              <details key={i} className="group rounded-xl border border-white/5 bg-white/5 overflow-hidden">
                <summary className="flex cursor-pointer items-center justify-between p-4 font-bold text-sm text-white select-none hover:bg-white/5 transition">
                  {item.q}
                  <span className="text-[#CC0000] transition-transform group-open:rotate-180">▼</span>
                </summary>
