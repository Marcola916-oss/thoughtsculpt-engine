...
function TopBar() {
  ...
  return (
    <header className={`sticky top-0 z-40 transition-all duration-300 ${scrolled ? "bg-background/90 backdrop-blur-md border-b border-border shadow-sm py-2" : "bg-transparent py-4"}`}>
      <div className="mx-auto flex max-w-6xl items-center justify-between px-4">
        ...
        <div className="flex items-center gap-4">
          <LanguageSwitcher />
          <Link to="/login" className="rounded-full px-4 py-2 text-sm font-semibold text-muted-foreground hover:bg-secondary hover:text-foreground transition">
            Log in
          </Link>
        </div>
      </div>
  ...
  return (
    <section className="py-12 md:py-24 text-center animate-in fade-in slide-in-from-bottom-8 duration-700">
      ...
      <p className="mx-auto mt-8 max-w-2xl text-lg text-muted-foreground md:text-xl leading-relaxed">
        {t.hero.sub} O primeiro diagnóstico comportamental impulsionado por IA que reconfigura sua forma de pensar. Sem planilhas, sem orçamentos chatos.
      </p>
      ...
      <div className="mt-12 flex flex-col items-center gap-4">
        ...
        <div className="flex items-center gap-2 text-xs text-muted-foreground mt-2">
          ...
          <span className="font-semibold text-foreground">14.392</span> avaliações
        </div>
      </div>
    </section>
  );
  ...
  return (
    <section className="py-12 animate-in fade-in slide-in-from-right-8 duration-500 max-w-xl mx-auto">
      ...
      <div className="mt-10 space-y-8">
        <div>
          <label className="mb-2 block text-sm font-bold uppercase tracking-wider text-muted-foreground">{t.common.yourName}</label>
          <input
            autoFocus
            value={props.name}
        ...
        <div>
          <label className="mb-2 block text-sm font-bold uppercase tracking-wider text-muted-foreground">{t.common.selectGender}</label>
    ...
    <section className="py-8 max-w-2xl mx-auto animate-in fade-in duration-300">
      <div className="mb-8">
        <div className="flex items-center justify-between mb-3">
          <button onClick={props.onBack} className="flex items-center gap-1 text-sm font-semibold text-muted-foreground hover:text-foreground transition">
        ...
        <div className="h-1.5 w-full overflow-hidden rounded-full bg-secondary">
          <div className="h-full bg-primary transition-[width] duration-500 ease-out shadow-[0_0_10px_var(--accent-glow)]" style={{ width: `${progress}%` }} />
        </div>
      </div>

      <h2 className="font-display text-3xl font-extrabold leading-tight md:text-4xl">{q.q.replace("[NOME]", props.name)}</h2>
        ...
        {q.options.map((opt, i) => (
          ...
          >
            ...
            <div className={`flex h-6 w-6 shrink-0 items-center justify-center rounded-full border-2 transition-colors ${
              props.selected === i ? "border-primary bg-primary" : "border-border bg-background group-hover:border-primary/50"
            }`}>
...
function EmailCapture(props: {
  ...
  return (
    <section className="py-12 max-w-xl mx-auto animate-in slide-in-from-bottom-8 duration-700">
      ...
      <h2 className="text-center font-display text-3xl font-extrabold md:text-5xl">{t.emailCapture.title(props.name)}</h2>
      <p className="mt-4 text-center text-lg text-muted-foreground leading-relaxed">{t.emailCapture.sub}</p>
      ...
      >
        ...
        <label className="flex items-start gap-3 rounded-xl border border-border bg-card/50 p-4 text-sm text-muted-foreground transition hover:bg-card cursor-pointer">
          <input type="checkbox" checked={props.gdpr} onChange={(e) => props.setGdpr(e.target.checked)} className="mt-1 h-5 w-5 shrink-0 accent-primary" />
...
function LoaderScreen() {
  ...
  return (
    <section className="flex flex-col items-center justify-center py-32 text-center animate-in fade-in">
      ...
      <h2 className="mt-10 font-display text-2xl font-bold text-foreground">Processando suas respostas</h2>
      <p className="mt-3 text-lg font-medium text-muted-foreground transition-opacity animate-pulse">{t.loader.steps[step]}</p>
    </section>
  );
}
    ...
    <section className="py-12 md:py-20 animate-in zoom-in-95 duration-700">
      <div className="text-center">
        ...
        <h1 className="mt-2 font-display text-5xl font-extrabold leading-tight text-foreground md:text-7xl">
          <span className="text-primary">{text}</span><span className="animate-pulse text-primary">|</span>
        </h1>
        <p className="mx-auto mt-6 max-w-2xl text-xl font-medium text-muted-foreground">{a.tagline}</p>
      </div>
      ...
      <div className="mx-auto mt-16 max-w-3xl rounded-3xl border border-border bg-card p-8 md:p-12 shadow-xl">
        ...
        <ul className="space-y-4">
          {a.hooks.map((h, i) => (
            <li key={i} className="flex gap-4 rounded-xl border border-border bg-background p-5 text-lg transition-transform hover:scale-[1.02]">
    ...
    <section className="py-12 md:py-20 animate-in fade-in duration-700 max-w-3xl mx-auto">
      ...
      <div className="space-y-12">
        ...
        <div className="rounded-2xl border border-border bg-card p-8 shadow-sm">
          <h2 className="font-display text-2xl font-bold text-foreground mb-4">Você já tentou de tudo, certo?</h2>
          <p className="text-muted-foreground leading-relaxed mb-6">
            Planilhas do Excel. Aplicativos de orçamento. Promessas de ano novo. Mas o padrão do {a.name} sempre volta a assumir o controle nas horas de estresse, ansiedade ou euforia. Isso acontece porque o problema não é matemático, é comportamental.
          </p>
          <ul className="space-y-4">
            {t.sales.bullets.map((b, i) => (
              <li key={i} className="flex items-start gap-4">
                <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-primary/20 text-primary text-xs font-bold mt-1">✓</span>
        ...
        <div className="text-center max-w-2xl mx-auto">
          ...
          <p className="mt-4 text-lg text-muted-foreground leading-relaxed">
            {t.sales.whyBody} O MindReset utiliza TCC (Terapia Cognitivo-Comportamental) adaptada para finanças, combinada com IA para mapear seus gatilhos invisíveis.
          </p>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          <div className="rounded-xl border border-border bg-card p-6">
            ...
            <p className="text-muted-foreground text-sm">Um dossiê de 4 dimensões (Finanças, Profissional, Pessoal, Amor) detalhando seus pontos cegos.</p>
          </div>
          <div className="rounded-xl border border-border bg-card p-6">
            <div className="text-4xl mb-4">📅</div>
            <h4 className="font-bold text-xl mb-2">Matriz de Ação</h4>
            <p className="text-muted-foreground text-sm">Protocolo diário de 30 dias gerado por IA com micro-tarefas para religar seu cérebro.</p>
          </div>
          <div className="rounded-xl border border-border bg-card p-6">
            <div className="text-4xl mb-4">🧭</div>
            <h4 className="font-bold text-xl mb-2">Compass</h4>
            <p className="text-muted-foreground text-sm">Decodifique as pessoas da sua vida e saiba exatamente o que dizer para evitar conflitos.</p>
          </div>
          <div className="rounded-xl border border-border bg-card p-6">
            <div className="text-4xl mb-4">📈</div>
            <h4 className="font-bold text-xl mb-2">Gamificação Real</h4>
            <p className="text-muted-foreground text-sm">Ganhe pontos, mantenha o streak e gere relatórios mensais para provar sua evolução.</p>
          </div>
        </div>
          ...
          return (
            <div
              ...
              className={`relative flex flex-col rounded-3xl border bg-card p-8 transition-all hover:-translate-y-2 ${
                popular ? "border-primary shadow-[0_0_40px_var(--accent-glow)] md:scale-105 z-10" : "border-border"
              }`}
            >
              {popular && (
              ...
              <div className="mb-6 flex-1 text-center">
                ...
                <div className="mt-6 flex items-baseline justify-center gap-1">
                  <span className="font-display text-5xl font-extrabold">{formatPrice(currency, total)}</span>
                </div>
                <p className="mt-2 text-sm font-semibold text-primary">{t.plans.perDay(pricePerDay(currency, p))}</p>
              </div>

              <div className="mb-8 space-y-3 border-t border-border pt-6">
                <div className="flex text-sm text-muted-foreground"><span className="mr-2 text-primary">✓</span> Diagnóstico Completo AI</div>
                <div className="flex text-sm text-muted-foreground"><span className="mr-2 text-primary">✓</span> Matriz de Ação Diária</div>
                <div className="flex text-sm text-muted-foreground"><span className="mr-2 text-primary">✓</span> Compass (Acessos ilimitados)</div>
                {p !== "30d" && <div className="flex text-sm font-semibold text-foreground"><span className="mr-2 text-primary">✓</span> Gamificação & Relatórios de IA</div>}
              </div>
      ...
      <div className="mt-24 max-w-3xl mx-auto text-left">
        ...
        <div className="space-y-4">
          <div className="rounded-xl border border-border bg-card p-6">
            ...
            <p className="text-muted-foreground mt-2">Não. Não pedimos senhas de banco nem mandamos você cortar o cafezinho. O MindReset foca na origem do problema: seu comportamento invisível e emocional com o dinheiro.</p>
          </div>
          <div className="rounded-xl border border-border bg-card p-6">
            <h4 className="font-bold text-lg">Posso cancelar a qualquer momento?</h4>
            <p className="text-muted-foreground mt-2">Sim. Direto pelo painel, com 2 cliques. Sem perguntas chatas ou ligações para retenção.</p>
          </div>
          <div className="rounded-xl border border-border bg-card p-6">
            <h4 className="font-bold text-lg">Como funciona a Inteligência Artificial?</h4>
            <p className="text-muted-foreground mt-2">A nossa IA usa as regras da TCC (Terapia Cognitivo-Comportamental) cruzadas com o seu arquétipo. Ela não escreve textos genéricos, ela desenha tarefas reais, como "Hoje, você vai atrasar essa compra impulsiva por 24h anotando-a neste espaço".</p>
          </div>
        </div>
      </div>
    </section>
    ...
    <footer className="mt-12 border-t border-border bg-card">
      <div className="mx-auto flex max-w-6xl flex-wrap items-center justify-between gap-4 px-4 py-8 text-sm text-muted-foreground">
        ...
        <div className="flex gap-6">
          <Link to="/privacy" className="hover:text-primary transition">Política de Privacidade</Link>
          <Link to="/terms" className="hover:text-primary transition">Termos de Uso</Link>
        </div>
      </div>
    </footer>
  );
