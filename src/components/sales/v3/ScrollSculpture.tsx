...
export function ScrollSculpture({ archetype, targetRef, className = "" }: Props) {
  ...
  const crackProg    = useTransform(smooth, [0.10, 0.40], [0, 1]);
  const crackGlow    = useTransform(smooth, [0.25, 0.55], [0, 1]);
  const shardOffset  = useTransform(smooth, [0.40, 0.58], [0, 1]);
  const reassembly   = useTransform(smooth, [0.82, 0.95], [1, 0]);
  const coreReveal   = useTransform(smooth, [0.58, 0.70], [0, 1]);
  ...
  useMotionValueEvent(smooth, "change", (v) => { scrollNowRef.current = v; });
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      const intensity = Math.max(0, Math.min(1, (sy - 0.55) / 0.30)) * Math.max(0, Math.min(1, (0.95 - sy) / 0.10));
    ...
    >
      ...
      {cracks.length > 0 && (
        <div className="absolute inset-0 flex items-center justify-center">
          <svg viewBox="0 0 200 200" className="h-[60vh] max-h-[540px] w-[60vh] max-w-[540px]" style={{ overflow: "visible" }}>
            <defs>
      ...
      >
        ...
        >
          <svg viewBox="0 0 200 200" className="h-full w-full" style={{ filter: "drop-shadow(0 0 14px color-mix(in oklab, var(--arch-primary) 70%, transparent))" }}>
            {sigil.map((el, i) => {
              const stroke = "currentColor";
              const sw = el.strokeWidth ?? 1;
              if (el.kind === "circle") {
                return <circle key={i} cx={el.cx} cy={el.cy} r={el.r} fill={el.fill ?? "none"} stroke={stroke} strokeWidth={sw} />;
              }
              if (el.kind === "polygon") {
                return <polygon key={i} points={el.points} fill={el.fill ?? "none"} stroke={stroke} strokeWidth={sw} />;
              }
              return <path key={i} d={el.d} fill={el.fill ?? "none"} stroke={stroke} strokeWidth={sw} strokeLinecap="round" strokeLinejoin="round" />;
            })}
          </svg>
        </motion.div>
      </motion.div>
      ...
      >
        <div
          ...
          style={{ background: "radial-gradient(circle, color-mix(in oklab, var(--arch-primary) 55%, transparent) 0%, transparent 70%)" }}
        />
      </motion.div>
