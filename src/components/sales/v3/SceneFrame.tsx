...
export function SceneFrame({
  ...
  return (
    <section
      ref={ref}
      data-scene={sceneId}
      aria-labelledby={`scene-heading-${sceneId}`}
      className="relative py-20 sm:py-28 w-full transition-all group"
    >
      {index && (
        <span
          className="sales-roman pointer-events-none opacity-20 group-hover:opacity-30 transition-opacity"
          aria-hidden
        >
          {ROMAN[index] ?? index}
        </span>
      ...
      <div className="relative z-10">
        <Reveal>
          ...
          {title && (
            <h2
              id={`scene-heading-${sceneId}`}
              className="mb-8 font-display font-extrabold uppercase leading-[1.02] tracking-tight text-white drop-shadow-md"
          ...
          <div
            className={
              dropCap ? "sales-dropcap text-white/90 text-[17px] leading-[1.7]" : "text-white/90"
            }
          >
            {children}
          </div>
