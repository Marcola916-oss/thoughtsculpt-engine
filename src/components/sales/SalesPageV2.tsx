/**
 * SalesPageV2 v5 — "The Awakening Protocol"
 *
 * Cinematic scroll-driven sales page: 6-act narrative with particle morphing,
 * neural mesh, SVG scene elements, and scroll-synchronized transitions.
 *
 * Same export + props as previous version.
 */
...
import { AwakeningScroll } from "./v3/AwakeningScroll";
import { ActContent } from "./v3/ActContent";
...
const ARCH_PRIMARY: Record<
  Archetype,
  { pt: string; en: string; pl: string; ro: string; ar: string }
> = {
  AO: {
    pt: "Acumulador Obsessivo",
    en: "Obsessive Saver",
    pl: "Skrupulatny Oszczędny",
    ro: "Acumulator Obsesiv",
    ar: "المدّخر القهري",
  },
  SS: {
    pt: "Buscador de Status",
    en: "Status Seeker",
    pl: "Łowca Statusu",
...
> = {
  pt: {
    pain: "O PROBLEMA",
    science: "A CIÊNCIA",
    fourD: "DIAGNÓSTICO REVELADO",
    deliver: "O QUE VAIS RECEBER",
    proof: "DEPOIMENTOS REAIS",
    faq: "DÚVIDAS FREQUENTES",
    decision: "A TUA DECISÃO",
  },
  en: {
    pain: "THE PROBLEM",
    science: "THE SCIENCE",
    fourD: "DIAGNOSIS REVEALED",
  ...
  pl: {
    pain: "PROBLEM",
    science: "NAUKA",
    fourD: "UJAWNIONA DIAGNOZA",
    deliver: "CO OTRZYMASZ",
    proof: "PRAWDZIWE OPINIE",
    faq: "CZĘSTE PYTANIA",
  ...
  ro: {
    pain: "PROBLEMA",
    science: "ȘTIINȚA",
    fourD: "DIAGNOSTIC DEZVĂLUIT",
    deliver: "CE PRIMEȘTI",
    proof: "MĂRTURII REALE",
  ...
  ar: {
    pain: "المشكلة",
    science: "العلم",
    fourD: "التشخيص الكامل",
    deliver: "ما ستحصل عليه",
    proof: "شهادات حقيقية",
    faq: "أسئلة شائعة",
    decision: "قرارك",
  },
...
export default function SalesPageV2({
  ...
  const primaryLabel =
    ARCH_PRIMARY[archetype][lang as "pt" | "en" | "pl" | "ro" | "ar"] ?? ARCH_PRIMARY[archetype].en;
  const secondaryLabel =
    ARCH_PRIMARY[ARCH_SECONDARY[archetype]][lang as "pt" | "en" | "pl" | "ro" | "ar"] ??
    ARCH_PRIMARY[ARCH_SECONDARY[archetype]].en;
  ...
  useEffect(() => {
    ...
    const obsHero = new IntersectionObserver(([e]) => setShowSticky(!e.isIntersecting), {
      threshold: 0.1,
    });
  ...
  // VSL_SCENE_VIEW for testimonials (no SceneFrame wrapper).
  useEffect(() => {
    if (typeof IntersectionObserver === "undefined") return;
    ...
    const obs = new IntersectionObserver(
      ([e]) => {
        if (e.isIntersecting && !fired) {
          fired = true;
          track(EVENTS.VSL_SCENE_VIEW, { scene: "testimonials" });
          obs.disconnect();
        }
      },
      { threshold: 0.35 },
  ...
  const advance = (source: string) => {
    track(EVENTS.VSL_CTA_CLICK, { arch: archetype, source });
    onContinue({ bumps: [] });
  };
  ...
  return (
    <div
      ...
      data-arch={archetype}
      data-awakening-root
      className="relative min-h-[700vh] text-white/90 selection:bg-[var(--arch-primary)] selection:text-white"
    >
      <AwakeningScroll rootRef={rootRef} archetype={archetype}>
        {/* ─── ACT I — O Vazio (0–12%) ───── */}
        <ActContent actId="void" sceneId="hero">
          <div ref={heroRef} className="mx-auto max-w-4xl px-5 sm:px-8 lg:px-16 pt-20 pb-32">
            <HeroScene
              ...
              proofs={[
                {
                  value: "+12.000",
                  label: v2.b6.counter.replace(/[+\d.,\s]+/g, " ").trim() || "Diagnoses",
                },
                {
                  value: "4.9★",
                  label: v2.b6.rating.replace(/[★⭐\d.,/\s]+/g, " ").trim() || "Rating",
                },
                { value: "60s", label: "PDF" },
        ...
        {/* ─── ACT II — O Despertar (12–25%) ───── */}
        <ActContent actId="awakening" sceneId="pain">
          <div className="mx-auto max-w-4xl px-5 sm:px-8 lg:px-16 py-20 sm:py-28">
            <SceneFrame sceneId="pain" index={1} badge={badges.pain} title={tpl(v2.b2.title)}>
        ...
        {/* ─── ACT III — O Mapa (30–50%) ───── */}
        <ActContent actId="map" sceneId="4d-panels">
          <div className="mx-auto max-w-4xl px-5 sm:px-8 lg:px-16 py-20 sm:py-28">
            ...
            <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
              {AREA_ORDER.map((area) => {
                const feat = v2.b4.features[AREA_ORDER.indexOf(area)];
                return (
                  <AreaPoster
                    key={area}
                    area={area}
        ...
        {/* ─── ACT IV — O Protocolo (50–67%) ───── */}
        <ActContent actId="protocol" sceneId="deliver">
          <div className="mx-auto max-w-4xl px-5 sm:px-8 lg:px-16 py-20 sm:py-28">
            <SceneFrame sceneId="deliver" badge={badges.deliver} title={tpl(v2.b5.title)}>
              ...
              <p className="mt-6 text-center text-xs text-white/60 font-semibold">{v2.b5.note}</p>
            </SceneFrame>
          </div>
        </ActContent>

        {/* ─── ACT V — A Prova (67–83%) ───── */}
        <ActContent actId="proof" sceneId="testimonials">
          <div
            ref={testimonialsRef}
            className="mx-auto max-w-4xl px-5 sm:px-8 lg:px-16 py-20 sm:py-28"
          >
            <SalesTestimonials
        ...
        {/* ─── ACT VI — A Decisão (83–100%) ───── */}
        <ActContent actId="decision" sceneId="decision">
          <div className="mx-auto max-w-4xl px-5 sm:px-8 lg:px-16 py-20 sm:py-28">
      ...
      <StickyOfferBar show={showSticky} cta={v2.b1.cta} onCta={() => advance("sticky")} />
